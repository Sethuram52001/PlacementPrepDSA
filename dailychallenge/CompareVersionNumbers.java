...
treat the revision as 0. For example, version 1.0 is less than version 1.1 because their revision 0s are the same, but their revision 1s 
are 0 and 1 respectively, and 0 < 1.

Link: https://leetcode.com/problems/compare-version-numbers/

Solution:
We can just do 1 pass iteration with Integer.compareTo(Integer).
*/
...
public class CompareVersionNumbers {
    public int compareVersion(String version1, String version2) {
        ...
        for(int i = 0; i < len; i++) {
            Integer val1 = i >= levels1.length ? 0 : Integer.valueOf(levels1[i]);
            Integer val2 = i >= levels2.length ? 0 : Integer.valueOf(levels2[i]);
